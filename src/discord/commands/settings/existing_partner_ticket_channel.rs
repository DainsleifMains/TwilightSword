...
pub fn subcommand_definition() -> CommandOption {
	let channel_option = ChannelBuilder::new(
		...
		"The channel in which existing partner tickets are posted",
	)
	.channel_types([ChannelType::GuildForum])
	.required(true)
	.build();

	let get = SubCommandBuilder::new("get", "Gets the existing partner ticket channel");
	let set = SubCommandBuilder::new("set", "Sets the existing partner ticket channel").option(channel_option);
	let unset = SubCommandBuilder::new("unset", "Removes the existing partner ticket channel");

	SubCommandGroupBuilder::new(
		"existing_partner_ticket_channel",
		"Manages the channel to which existing partner tickets are posted",
	)
...
async fn set_ticket_channel(
	...
	let Some(existing_partner_ticket_channel) = values.first() else {
		...
		"The only option for `/settings existing_partner_ticket_channel set` should be `existing_partner_ticket_channel`"
	);
