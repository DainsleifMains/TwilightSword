...
pub async fn handle_subcommand(
	...
	let CommandOptionValue::SubCommandGroup(value_data) = subcommand_value else {
		bail!(
			"Command data is malformed; expected `/settings new_partner_ticket_channel` to get a subcommand group value"
		);
	};
...
async fn set_ticket_channel(
	...
	let Some(new_partner_ticket_channel) = values.first() else {
		bail!(
			"Command data is malformed; expected `/settings new_partner_ticket_channel set` to have required option `new_partner_ticket_channel`"
		);
	};
	ensure!(
	...
	let CommandOptionValue::Channel(new_partner_ticket_channel) = new_partner_ticket_channel.value else {
		bail!(
			"Command data is malformed; expected `new_partner_ticket_channel` option of `/settings new_partner_ticket_channel set` to be a channel"
		);
	};
