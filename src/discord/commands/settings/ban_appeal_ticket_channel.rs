...
pub fn subcommand_definition() -> CommandOption {
	let channel_option = ChannelBuilder::new(
		...
		"The channel in which ban appeal tickets are posted",
	)
	.channel_types([ChannelType::GuildForum])
	.required(true)
	.build();

	let get = SubCommandBuilder::new("get", "Gets the ban appeal ticket channel");
	let set = SubCommandBuilder::new("set", "Sets the ban appeal ticket channel").option(channel_option);
	let unset = SubCommandBuilder::new("unset", "Removes the ban appeal ticket channel");

	SubCommandGroupBuilder::new(
		"ban_appeal_ticket_channel",
		"Manages the channel to which ban appeal tickets are posted",
	)
...
async fn get_ticket_channel(
	...
	let channel = guild.get_ban_appeal_ticket_channel();

	let interaction_client = http_client.interaction(application_id);
	let response_content = match channel {
		Some(channel) => format!("The ban appeal ticket channel is set up as {}.", channel.mention()),
		None => String::from("No ban appeal ticket channel is set."),
	};
	let response = InteractionResponseDataBuilder::new()
...
async fn set_ticket_channel(
	...
	let Some(ban_appeal_ticket_channel) = values.first() else {
		...
		"The only option for `/settings ban_appeal_ticket_channel set` should be `ban_appeal_ticket_channel`"
	);
	...
	if !permissions_in_channel.contains(
		...
		let response_content = format!(
			"The channel {} doesn't have the necessary permissions (Send Messages In Threads, Create Public Threads, Manage Threads) for me to update it.",
			ban_appeal_ticket_channel.mention()
		);
	...
	let db_channel_id = database_id_from_discord_id(ban_appeal_ticket_channel.get());

	let db_result = diesel::update(guilds::table)
		.filter(guilds::guild_id.eq(guild.guild_id))
		.set(guilds::ban_appeal_ticket_channel.eq(Some(db_channel_id)))
		.execute(db_connection);
	match db_result {
		Ok(_) => {
			let response = InteractionResponseDataBuilder::new()
				.content(format!(
					"Updated the ban appeal ticket channel to {}.",
					ban_appeal_ticket_channel.mention()
				))
				.allowed_mentions(AllowedMentions::default())
				.build();
		...
		Err(error) => {
			tracing::error!(source = ?error, "Failed to update the ban appeal ticket channel for a server");
			let response = InteractionResponseDataBuilder::new()
...
async fn unset_ticket_channel(
	...
	let original_channel = guild.get_ban_appeal_ticket_channel();
	let interaction_client = http_client.interaction(application_id);
	let response = match original_channel {
		Some(_) => {
			...
			let db_result = diesel::update(guilds::table)
				.filter(guilds::guild_id.eq(guild.guild_id))
				.set(guilds::ban_appeal_ticket_channel.eq(no_id))
				.execute(db_connection);
			match db_result {
				Ok(_) => InteractionResponseDataBuilder::new()
					.content("The ban appeal ticket channel has been unset.")
					.build(),
				Err(error) => {
					tracing::error!(source = ?error, "Failed to remove the ban appeal ticket channel for a server");
					InteractionResponseDataBuilder::new()
						.content("An internal error occurred, so the ban appeal ticket channel couldn't be unset.")
						.flags(MessageFlags::EPHEMERAL)
						.build()
				}
			}
