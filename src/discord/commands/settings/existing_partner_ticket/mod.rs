...
pub fn subcommand_definition() -> CommandOption {
	let channel_option = ChannelBuilder::new("existing_partner_ticket_channel", "Existing partner ticket settings")
		.channel_types([ChannelType::GuildForum])
		.required(true)
		.build();

	let channel_get = SubCommandBuilder::new("channel_get", "Gets the existing partner ticket channel");
	let channel_set =
		SubCommandBuilder::new("channel_set", "Sets the existing partner ticket channel").option(channel_option);
	let channel_unset = SubCommandBuilder::new("channel_unset", "Removes the existing partner ticket channel");

	let form_get = SubCommandBuilder::new("form_get", "Gets the existing partner ticket form");
	let form_set = SubCommandBuilder::new("form_set", "Sets the existing partner ticket form");
	let form_unset = SubCommandBuilder::new("form_unset", "Removes the existing partner ticket form");

	SubCommandGroupBuilder::new("existing_partner_ticket", "Existing partner ticket settings")
...
pub async fn handle_subcommand(
	...
	let guild = match guild {
		...
		Err(error) => {
			tracing::error!(source = ?error, "Failed to retrieve guild for getting or updating existing partner ticket channel");
			let response = InteractionResponseDataBuilder::new()
	...
	match value.name.as_str() {
		...
		_ => bail!(
			"Unknown settings existing_partner_ticket_channel subcommand encountered: {}\n{:?}",
			value.name,
			subcommand_value
		),
	}
