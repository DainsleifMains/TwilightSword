...
use crate::discord::state::settings::new_partner_ticket_form_set::{
	NewPartnerFormAssociationData, NewPartnerFormAssociations, new_partner_form_association_components,
};
...
/// Sets the form to use for new partner tickets
pub async fn execute(
	...
	let components = new_partner_form_association_components(&session_id, &guild_forms, None, 0);
	...
	let session_data = NewPartnerFormAssociationData {
		guild_id,
...

	{
		...
		let all_sessions = state.entry().or_insert_with(NewPartnerFormAssociations::default);
...
async fn expire_session(bot_state: Arc<RwLock<TypeMap>>, session_id: String) {
	...
	let mut state = bot_state.write().await;
	let all_sessions = state.get_mut::<NewPartnerFormAssociations>();
