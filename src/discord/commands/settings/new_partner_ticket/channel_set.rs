...
pub async fn execute(
	...
	let CommandOptionValue::SubCommand(values) = subcommand_value else {
		bail!("Command data is malformed; expected `/settings new_partner_ticket channel_set` to get subcommand data");
	};
	let Some(new_partner_ticket_channel) = values.first() else {
		bail!(
			"Command data is malformed; expected `/settings new_partner_ticket channel_set` to have required option `new_partner_ticket_channel`"
		);
	};
	ensure!(
		new_partner_ticket_channel.name.as_str() == "new_partner_ticket_channel",
		"The only option for `/settings new_partner_ticket channel_set` should be `new_partner_ticket_channel`"
	);

	let CommandOptionValue::Channel(new_partner_ticket_channel) = new_partner_ticket_channel.value else {
		bail!(
			"Command data is malformed; expected `new_partner_ticket_channel` option of `/settings new_partner_ticket channel_set` to be a channel"
		);
	};
	...
	if !permissions_in_channel.contains(ticket_channel_permissions()) {
		let response_content = ticket_channel_missing_permissions_message(new_partner_ticket_channel.mention());
