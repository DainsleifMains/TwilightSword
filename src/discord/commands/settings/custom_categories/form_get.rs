...
use crate::discord::state::settings::custom_categories_form_get::{
	CustomCategoryFormRetrievalData, CustomCategoryFormRetrievals, custom_category_form_retrieval_components,
};
...
pub async fn execute(
	...
	let components = custom_category_form_retrieval_components(&session_id, &all_categories, None, 0);
	...
	let new_data = CustomCategoryFormRetrievalData {
		all_categories,
		current_page: 0,
	};

	{
		...
		let data_sessions = state.entry().or_insert_with(CustomCategoryFormRetrievals::default);
		data_sessions.sessions.insert(session_id.clone(), new_data);
	}
...
async fn expire_session(bot_state: Arc<RwLock<TypeMap>>, session_id: String) {
	...
	let mut state = bot_state.write().await;
	if let Some(data_sessions) = state.get_mut::<CustomCategoryFormRetrievals>() {
